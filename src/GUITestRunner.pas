...
uses
  ...
  Classes, IniFiles, ToolWin, dcntree, dcTreeGrid;
...
type
  ...
  TGUITestRunner = class(TForm, ITestListener, ITestListenerX)
    ...
    procedure TestCaseIgnoreSetUpTearDownLeaksMenuItemDrawItem(Sender: TObject;
      ACanvas: TCanvas; ARect: TRect; Selected: Boolean);
    procedure TestTreeChange(Sender: TObject; Node: TDCTreeNode);
  private
  ...
  protected
    ...
    procedure DisplayFailureMessage(Item: TDCTreeGridNode);
    ...
    procedure UpdateNodeImage(node: TDCTreeNode);
    procedure UpdateNodeState(node: TDCTreeNode);
    procedure SetNodeState(node: TDCTreeNode; enabled :boolean);
    procedure SwitchNodeState(node: TDCTreeNode);
    procedure UpdateTestTreeState;

    procedure MakeNodeVisible(node :TDCTreeNode);
    procedure SetTreeNodeImage(Node :TDCTreeNode; imgIndex :Integer);
    procedure SelectNode(node: TDCTreeNode);

    function NodeToTest(node: TDCTreeNode): ITest;
    function  TestToNode(test :ITest) :TDCTreeNode;
    ...
    procedure ApplyToTests(root :TDCTreeNode; const func :TTestFunc);
    ...
    procedure CollapseNonGrandparentNodes(RootNode: TDCTreeNode);

    procedure ProcessClickOnStateIcon;
    procedure ClearStatusMessage;

    procedure CopyTestNametoClipboard(ANode: TDCTreeNode);

    procedure SetupCustomShortcuts;
    procedure SetupGUINodes;

    function SelectNodeIfTestEnabled(ANode: TDCTreeNode): boolean;
...
function TGUITestRunner.NodeToTest(node: TDCTreeNode): ITest;
var
  idx: Integer;
begin
...
function TGUITestRunner.TestToNode(test: ITest): TDCTreeNode;
begin
  assert(assigned(test));

  Result := test.GUIObject as TDCTreeNode;
...
procedure TGUITestRunner.UpdateNodeState(node: TDCTreeNode);
var
  test: ITest;
begin
  assert(assigned(node));
...
procedure TGUITestRunner.SetNodeState(node: TDCTreeNode; enabled :boolean);
var
  MostSeniorChanged :TDCTreeNode;
begin
   assert(node <> nil);
...
procedure TGUITestRunner.SwitchNodeState(node: TDCTreeNode);
begin
   assert(node <> nil);
...
begin
  ...
  begin
    ...
    if Assigned(Suite) then
      (ResultsView.Items[0] as TDCTreeGridNode).Columns[0] := IntToStr(FTotalTestsCount)
    else
      (ResultsView.Items[0] as TDCTreeGridNode).Columns[0] := '';
  end;
  ...
  begin
    ...
    begin
      with ResultsView.Items[0] as TDCTreeGridNode do
      begin
        Columns[1] := IntToStr(TestNumber);
        Columns[2] := IntToStr(TestResult.failureCount);
        Columns[3] := IntToStr(TestResult.errorCount);
        Columns[4] := IntToStr(TestResult.Overrides);
        Columns[5] := FormatElapsedTime(TestResult.TotalTime);
        Columns[6] := FormatElapsedTime(max(TestResult.TotalTime, FTotalTime));
      end;
      with TestResult do
      begin
  ...
  begin
    with ResultsView.Items[0] as TDCTreeGridNode do
    begin
      if (Columns[0] = '0') or (Columns[0] = '') then
      begin
        for i := 1 to 6 do
          Columns[i] := ''
      end
      ...
      begin
        if Columns[0] <> Columns[1] then
          for i := 1 to 6 do
            Columns[i] := ''
        else
        begin
          Columns[5] := FormatElapsedTime(SelectedTest.ElapsedTestTime);
          Columns[6] := FormatElapsedTime(Max(SelectedTest.ElapsedTestTime, FTotalTime));
        end;
      end;
    end;
...
function TGUITestRunner.AddFailureItem(failure: TTestFailure): TDCTreeGridNode;
var
  item : TDCTreeGridNode;
  node : TDCTreeNode;
begin
  assert(assigned(failure));
  item := FailureListView.Items.Add(nil, '') as TDCTreeGridNode;
  item.data := Pointer(TestToNode(failure.failedTest));
  item.Text := failure.failedTest.Name;
  item.Columns[1] := failure.thrownExceptionName;
  item.Columns[2] := failure.thrownExceptionMessage;
  item.Columns[3] := failure.LocationInfo
                     + ' ' +
                     failure.AddressInfo;
  item.Columns[4] := failure.StackTrace;
...
procedure TGUITestRunner.FillTestTree(RootNode: TDCTreeNode; ATest: ITest);
var
...
procedure TGUITestRunner.SetTreeNodeImage(Node :TDCTreeNode; imgIndex :Integer);
begin
  while Node <> nil do
  begin
...
procedure TGUITestRunner.DisplayFailureMessage(Item: TDCTreeGridNode);
var
  hlColor :TColor;
  Test    :ITest;
  Status  :string;
begin
  TestTree.Selected := TDCTreeNode(Item.data);
  ...
  with ErrorMessageRTF do
    begin
      ...
      SelAttributes.Style := [fsBold];
      SelText := Item.Columns[1];

      Lines.Add('');
      SelAttributes.Color := clWindowText;
      SelAttributes.Style := [];
      SelText := 'at ' + Item.Columns[3];

      if Item.Columns[2] <> '' then
      begin
        SelAttributes.Color := clWindowText;
        Lines.Add('');
        SelAttributes.Size  := 12;
        SelAttributes.Style := [];
        SelText := Item.Columns[2];
        SelAttributes.Size  := self.Font.Size;
      end;
      ...
      if Item.Columns[4] <> '' then
      begin
        Lines.Add('');
        SelAttributes.Style := [fsBold];
        Lines.Add('StackTrace');
        SelAttributes.Style := [];
        SelText := Item.Columns[4];
      end;
    end
end;
...
begin
  ...
  begin
    TestTree.Selected := TDCTreeNode(FailureListView.Selected.data);
    DisplayFailureMessage(FailureListView.Selected as TDCTreeGridNode);
  end
  else ClearFailureMessage;
end;
...
procedure TGUITestRunner.ApplyToTests(root :TDCTreeNode; const func :TTestFunc);

  procedure DoApply(rootnode :TDCTreeNode);
  var
    test: ITest;
    node: TDCTreeNode;
  begin
...
begin
  ...
  begin
    ANode := TDCTreeNode(FailureListView.Items[i].Data);
...
function TGUITestRunner.NodeIsGrandparent(ANode: TDCTreeNode): boolean;
var
  AChildNode: TDCTreeNode;
begin
  Result := false;
...
procedure TGUITestRunner.CollapseNonGrandparentNodes(RootNode: TDCTreeNode);
var
  AChildNode: TDCTreeNode;
begin
...
procedure TGUITestRunner.UpdateNodeImage(node: TDCTreeNode);
var
  test :ITest;
begin
  test := NodeToTest(node);
...
procedure TGUITestRunner.CopyTestNametoClipboard(ANode: TDCTreeNode);
begin
...
function TGUITestRunner.SelectNodeIfTestEnabled(ANode: TDCTreeNode): boolean;
var
  ATest: ITest;
begin
  ATest := NodeToTest(ANode);
...
procedure TGUITestRunner.SelectNode(node: TDCTreeNode);
begin
  node.Selected := true;
  MakeNodeVisible(node);
end;
...
procedure TGUITestRunner.TestTreeChange(Sender: TObject; Node: TDCTreeNode);
var
  i : integer;
begin
  ...
  begin
    ...
    for i := 0 to FailureListView.Items.count - 1 do
    begin
      if TDCTreeNode(FailureListView.Items[i].Data) = Node then
      begin
        FailureListView.Selected := FailureListView.Items[i];
        FailureListViewClick(FailureListView);
        break;
      end;
    end;
    UpdateStatus(True);
