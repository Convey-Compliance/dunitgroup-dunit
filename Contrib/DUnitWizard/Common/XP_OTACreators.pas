...
{
 $Source: /cvsroot/dunit/dunit/Contrib/DUnitWizard/Common/XP_OTACreators.pas,v $
 $Revision: 1.1 $
...
const CVSID: string = '$Header: /cvsroot/dunit/dunit/Contrib/DUnitWizard/Common/XP_OTACreators.pas,v 1.1 2003/01/27 12:48:27 pvspain Exp $';
...
begin
  // Prompt with "Save As..."  when the user closes this module.
  Result := true;
end;
...
begin
  inherited Create;
  FAbsoluteUnitName := SysUtils.Trim(AUnitName);

  if FAbsoluteUnitName <> '' then
  begin
    // Ensure we have an absolute file specification for GetFileName()
    FAbsoluteUnitName := SysUtils.ExpandFileName(FAbsoluteUnitName);
    // Create as a PAS file
    SysUtils.ChangeFileExt(FAbsoluteUnitName, '.pas');
  end;
...
begin
  ...
  Result := SysUtils.ChangeFileExt(
    SysUtils.ExtractFileName(FAbsoluteUnitName), '') + 'Form';
end;
...
begin
  inherited Create;
  FAbsoluteProjectName := SysUtils.Trim(AProjectName);

  if FAbsoluteProjectName <> '' then
  begin
    // Ensure we have an absolute file specification for GetFileName()
    FAbsoluteProjectName := SysUtils.ExpandFileName(FAbsoluteProjectName);

    // Create as an application (or library) if extension not provided
    if SysUtils.ExtractFileExt(FAbsoluteProjectName) = '' then
      FAbsoluteProjectName := FAbsoluteProjectName + '.dpr';
...
function TXP_OTAProjectCreator.GetUnnamed: Boolean;
begin
  // Prompt with "Save As" if FProjectName is undefined
  Result := FAbsoluteProjectName = '';
end;
